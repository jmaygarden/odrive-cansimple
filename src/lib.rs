...
use crate::utils::{
    decode_id, FrameBuilder
};
use socketcan::{
    CANFrame, CANSocket, CANSocketOpenError
};
...
#[derive(Debug)]
pub enum Signal {
    EncoderCount{ id: AxisId, shadow_count: i32, cpr_count: i32 },
    EncoderError{ id: AxisId, error: EncoderError },
    EncoderEstimates{ id: AxisId, position: f32, velocity: f32 },
    Heartbeat{ id: AxisId, error: AxisError, state: AxisState },
    MotorCurrent{ id: AxisId, setpoint: f32, measurement: f32 },
    MotorError{ id: AxisId, error: MotorError },
    VbusVoltage{ id: AxisId, voltage: f32 },
    Unknown{ id: AxisId, command: Command },
}
...
impl Connection {
            ...
            Ok(frame) => {
                ...
                match command {
                    ...
                    Command::GetEncoderError => {
                        let error = match EncoderError::from_bits(reader.read_u32::<LittleEndian>().unwrap()) {
                            Some(value) => value,
                            None => EncoderError::ERROR_NONE,
                        };

                        Ok(Signal::EncoderError{ id: node, error: error })
                    },
                    ...
                    Command::OdriveHeartbeat => {
                        let error = match AxisError::from_bits(reader.read_u32::<LittleEndian>().unwrap()) {
                            Some(value) => value,
                            None => AxisError::ERROR_NONE,
                        };
                        let state = match num::FromPrimitive::from_u32(reader.read_u32::<LittleEndian>().unwrap()) {
                            Some(value) => value,
                    ...
                    Command::GetMotorError => {
                        let error = match MotorError::from_bits(reader.read_u32::<LittleEndian>().unwrap()) {
                            Some(value) => value,
                            None => MotorError::ERROR_NONE,
                        };

                        Ok(Signal::MotorError{ id: node, error: error })
                    }

                    Command::GetVbusVoltage => {
                        let voltage = reader.read_f32::<LittleEndian>().unwrap();

                        Ok(Signal::VbusVoltage{ id: node, voltage: voltage })
                    }

                    _ => Ok(Signal::Unknown{ id: node, command: command }),
                }
            },
            Err(error) => Err(error),
        }
    ...
    pub fn set_axis_node_id(&self, id: AxisId, new_id: AxisId) -> std::io::Result<()> {
        let frame = FrameBuilder::new(id as u32, Command::SetAxisNodeId)
                        .arg0(new_id as u32).finalize().unwrap();
    ...
    pub fn set_axis_requested_state(&self, id: AxisId, state: AxisState) -> std::io::Result<()> {
        let frame = FrameBuilder::new(id as u32, Command::SetAxisRequestedState)
                        .arg0(state as u32).finalize().unwrap();
    ...
    pub fn set_velocity_limit(&self, id: AxisId, value: f32) -> std::io::Result<()> {
        let frame = FrameBuilder::new(id as u32, Command::SetVelLimit)
                        .arg0(u32::from_le_bytes(value.to_le_bytes())).finalize().unwrap();

        self.write(&frame)
    }

    pub fn set_controller_modes(&self, id: AxisId, control_mode: ControlMode) -> std::io::Result<()> {
    ...
    fn request(&self, id: AxisId, command: Command) -> std::io::Result<()> {
        let frame = FrameBuilder::new(id as u32, command).rtr(true)
                        .finalize().unwrap();
